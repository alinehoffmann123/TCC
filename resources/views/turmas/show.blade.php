...
<div class="animate-fade-in">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h2 class="text-3xl font-bold text-gray-dark">Detalhes da Turma: <span class="text-bordo-dark">{{ $turma->nome }}</span></h2>
    ...
    <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-light animate-fade-in animate-delay-100">
                ...
                <div class="space-y-2 text-gray-dark">
                    ...
                    <p><strong>Modalidade:</strong> 
                        <span class="px-2 py-1 text-xs font-semibold rounded-full 
                            ...
                            @endphp
                        ">{{ ucfirst(str_replace('-', ' ', $turma->modalidade)) }}</span>
                    </p>
                    <p><strong>Nível:</strong> 
                        <span class="px-2 py-1 text-xs font-semibold rounded-full 
                            @php
                                switch ($turma->nivel) {
                                    case 'iniciante': echo 'bg-green-100 text-green-800'; break;
                                    case 'intermediario': echo 'bg-yellow-100 text-yellow-800'; break;
                                    case 'avancado': echo 'bg-red-100 text-red-800'; break;
                                    case 'misto': echo 'bg-blue-100 text-blue-800'; break;
                                }
                            @endphp
                        ">{{ ucfirst($turma->nivel) }}</span>
                    </p>
                        ...
                        <span class="px-2 py-1 text-xs font-semibold rounded-full 
                            @php
                                switch ($turma->status) {
                                    ...
                                    case 'inativa': echo 'bg-red-100 text-red-800'; break;
                                    case 'pausada': echo 'bg-yellow-100 text-yellow-800'; break;
                                }
                            @endphp
                        ">{{ ucfirst($turma->status) }}</span>
                    </p>
                </div>
            </div>
            <div>
                ...
                <div class="space-y-2 text-gray-dark">
                    ...
                    <p><strong>Capacidade Máxima:</strong> {{ $turma->capacidade_maxima }} alunos</p>
                    <p><strong>Alunos Atuais:</strong> {{ $turma->numero_alunos }}</p>
                    <p><strong>Ocupação:</strong> {{ $turma->ocupacao_percentual }}%</p>
                </div>
            </div>
        </div>
        ...
        <div class="mt-6 border-t pt-6">
            ...
            <p class="text-gray-dark">{{ $turma->observacoes }}</p>
        </div>
        @endif
        <div class="mt-6 border-t pt-6">
            <h4 class="text-lg font-semibold text-bordo-dark mb-3 flex items-center">
        ...
        <div class="mt-6 border-t pt-6 text-right">
            <a href="{{ route('turmas.edit', $turma->id) }}" class="bg-bordo-dark hover:bg-bordo-hover text-white font-semibold py-2 px-4 rounded-md shadow-md transition-colors duration-200 inline-flex items-center space-x-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
