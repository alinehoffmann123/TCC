...
<div class="animate-fade-in">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h2 class="text-3xl font-bold text-gray-dark">Gerenciar Alunos da Turma: <span class="text-bordo-dark">{{ $aTurmas->nome }}</span></h2>
    ...
    <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-light mb-6 animate-fade-in animate-delay-200">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-dark">
            <p><strong>Instrutor:</strong> {{ $aTurmas->instrutor }}</p>
            ...
            <p><strong>Capacidade:</strong> {{ $aTurmas->numero_alunos }} / {{ $aTurmas->capacidade_maxima }} ({{ $aTurmas->ocupacao_percentual }}%)</p>
            <p><strong>Modalidade:</strong> {{ ucfirst(str_replace('-', ' ', $aTurmas->modalidade)) }}</p>
            <p><strong>Nível:</strong> {{ ucfirst($aTurmas->nivel) }}</p>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-light mb-6 animate-fade-in animate-delay-300">
        <h3 class="text-xl font-semibold text-gray-dark mb-4 flex items-center">
            ...
            Alunos Atuais na Turma ({{ $aTurmas->alunos->count() }})
        </h3>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-light">
                    ...
                    <tbody class="bg-white divide-y divide-gray-light">
                        @foreach ($aTurmas->alunos as $aAluno)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10 rounded-full bg-bordo-dark flex items-center justify-center text-white font-bold text-sm">{{ substr($aAluno->nome, 0, 2) }}</div>
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-dark">{{ $aAluno->nome }}</div>
                                        <div class="text-sm text-gray-dark/70">Matrícula: #{{ $aAluno->id }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-dark/80">{{ $aAluno->email }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @php
                                    $sCorFaixa = '';
                                    switch ($aAluno->faixa) {
                                        case 'branca': $sCorFaixa = 'bg-gray-200 text-gray-dark';
                                        break;
                                        case 'azul': $sCorFaixa = 'bg-blue-100 text-blue-800'; 
                                        break;
                                        case 'roxa': $sCorFaixa = 'bg-purple-100 text-purple-800';
                                         break;
                                        case 'marrom': $sCorFaixa = 'bg-yellow-100 text-yellow-800'; 
                                        break;
                                        case 'preta': $sCorFaixa = 'bg-black text-white'; 
                                        break;
                                        default: $sCorFaixa = 'bg-gray-200 text-gray-dark'; 
                                    break;
                                    }
                                @endphp
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $sCorFaixa }}">{{ ucfirst($aAluno->faixa) }}</span>
                            </td>
                            @php($role = auth()->user()?->role)
                            @if(in_array($role, ['professor']))
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <form action="{{ route('turmas.remover-aluno', ['turma' => $aTurmas->id, 'aluno' => $aAluno->id]) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
    ...
    @if(in_array($role, ['professor']))
        <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-light animate-fade-in animate-delay-400">
            ...
            @if($aAlunosDisponiveis->count() > 0)
                <form action="{{ route('turmas.matricular', $aTurmas->id) }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        ...
                        >
                            ...
                            @foreach($aAlunosDisponiveis as $aAluno)
                                <option value="{{ $aAluno->id }}">{{ $aAluno->nome }} ({{ $aAluno->email }})</option>
                            @endforeach
                        </select>
                        @error('aluno_id')
